...
// Tambahkan state untuk data guru
const guruData = ref(null)

// Tambahkan computed property untuk cek role admin
const isAdmin = computed(() => {
  return guruData.value?.role === 'admin'
})

// Tambahkan fungsi fetch data guru
...
const savePerangkat = async (perangkatData) => {
  try {
    ...
    if (selectedPerangkat.value) {
      await api.put(`/perangkat-kurikulum/${selectedPerangkat.value.id}/`, perangkatData, config)
    } else {
      await api.post('/perangkat-kurikulum/', perangkatData, config)
    }
    ...
    const errorMessage = error.response?.data?.mata_pelajaran?.[0] || 
                        error.response?.data?.error || 
                        'Gagal menyimpan perangkat kurikulum'
    notify({
      type: 'error',
      title: 'Error',
...
// Lifecycle hooks
onMounted(async () => {
  await fetchGuruData() // Ini akan memanggil fetchPerangkat setelah mendapatkan data guru
